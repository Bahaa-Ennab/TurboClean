...
<body style="background-color: #cad1d1;">
	...
	<script>
		const waitingCount = Number('<c:out value="${waitingCount != null ? waitingCount : 0}" />');
		const inProgressCount = Number('<c:out value="${inProgressCount != null ? inProgressCount : 0}" />');
		const finishedCount = Number('<c:out value="${finishedCount != null ? finishedCount : 0}" />');
		const paidCount = Number('<c:out value="${paidCount != null ? paidCount : 0}" />');
		...
		const chart = new Chart(ctx, {
			...
			data : {
				...
				datasets : [ {
					data : [ waitingCount, inProgressCount, finishedCount,
							paidCount ],
					backgroundColor : [ 'silver', '#64d0ea', '#ff8800',
							'#4CAF50' ],
					borderWidth : 1
				} ]
			},
